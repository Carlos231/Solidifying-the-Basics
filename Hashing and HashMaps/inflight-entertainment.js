...
function canTwoMoviesFillFlight(movieLengths, flightLength) {
  ...
  /*
  O(n) time and space overall
    convert movieLengths to a set
    loop through the movieLengths array
      calculate time neeeded from array value and flightTime
      check if this time is in the set created (takes care of multiples)
        return true
    update the movieLenths set with the array item not in it
  */

  const movieLengthSet = new Set(); //O(n)

  for (let i = 0; i < movieLengths.length; i++) {

    const timeNeeded = flightLength - movieLengths[i];

    if (movieLengthSet.has(timeNeeded)) {
      return true;
    }
