...
function isFirstComeFirstServed(takeOutOrders, dineInOrders, servedOrders) {
    ...
    /*
      hold index for takeout at 0
      hold max length for takeout
      hold index for dine in at 0
      hold max lenfth for dine in
      loop through the served orders
        make sure havent gone through either arrays lengths
          see which array the order is in
          increment indexes as go through served seeing that they are in order
          if the item is in neither arrays current index
            return false
      check if there are extra orders in either list and return false
      
      all orders accounted for so return true
          
    */

    let takeoutIndex = 0;
    let dineInIndex = 0;
    const takeoutLength = takeOutOrders.length - 1;
    const dineInLength = dineInOrders.length - 1;

    for (let i = 0; i < servedOrders.length; i++) {
        var order = servedOrders[i];

        if (takeoutIndex <= takeoutLength && takeOutOrders[takeoutIndex] === order) {
            takeoutIndex++;
        } else if (dineInIndex <= dineInLength && order === dineInOrders[dineInIndex]) {
            dineInIndex++;
        } else {
            return false;
        }
